/*
  Seabee Activity Schema — Milestone 3
  Safe/idempotent migration.

  Creates 7 tables that form the Seabee estimate hierarchy:

    1. production_rate_import_batches   — data governance / traceability
    2. production_rates                 — manual labor rates (global, read-only)
    3. construction_activity_templates  — Seabee activity templates (global)
...
CREATE TABLE IF NOT EXISTS pricing_library (
  id                   uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id           uuid,                           -- NULL = Seabee default/global
