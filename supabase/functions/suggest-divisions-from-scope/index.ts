...
const FALLBACK_DIVISION_CONFIDENCE = 0.75;
...
function keywordFallbackDivisions(
  ...
  const divisions: DivisionSuggestionItem[] = divisionCodes.map((code) => {
    ...
    return {
      divisionCode: code,
      divisionName,
      confidence: FALLBACK_DIVISION_CONFIDENCE,
  ...
  const notes = ["AI returned no valid divisions. Keyword fallback was used."];
...
function sanitizeDivisions(
  ...
  for (const item of rawDivisions) {
    ...
    const confidence = Number.isFinite(confidenceRaw)
      ...
      : DEFAULT_DIVISION_CONFIDENCE;
    if (confidence < MIN_DIVISION_CONFIDENCE) continue;
...
- Do not return divisions with confidence below 0.35.
...
serve(async (req) => {
  ...
  try {
    ...
    const promptPayload = {
      ...
      estimateType: typeof body.estimateType === "string" ? body.estimateType.trim() : "",
    };
    ...
    console.error("suggest-divisions-from-scope:", err);
