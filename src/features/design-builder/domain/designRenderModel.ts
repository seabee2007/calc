...
function renderMeter(value: number): number {
...
function componentKind(type: DesignComponentType): DesignRenderRcComponentKind | null {
  if (
    type === 'column' ||
    type === 'footer' ||
    type === 'tie_beam' ||
...
function componentElevations(component: PlacedDesignComponent): { base: number; top: number } {
...
function componentDimensions(component: PlacedDesignComponent): DesignRenderRcComponent['dimensions'] {
  const p = component.parameters;
  if (component.type === 'slab') {
...
function columnFooter(component: PlacedDesignComponent, width: number, depth: number, base: number): DesignRenderFooterReference | undefined {
  const p = component.parameters;
...
export function buildDesignRenderModel(params: {
  ...
  const rcComponents = (params.placedComponents ?? [])
