...
export default function DesignBuilderPage({
  ...
  function splitPlumbingRunAtSegment(
  ...
  ): { system: PlumbingSystem; nodeId: string; point: PlumbingPoint3D; createdNodeKind: PlumbingNodeKind } | null {
    ...
    if (existingNode) {
      return { system, nodeId: existingNode.id, point: existingNode.position, createdNodeKind: existingNode.kind };
    }
    if (pointsNearlyEqual(run.path[0]!, snap.point)) {
      const node = system.nodes.find((candidate) => candidate.id === run.startNodeId);
      if (node) return { system, nodeId: run.startNodeId, point: node.position, createdNodeKind: node.kind };
    }
    if (pointsNearlyEqual(run.path[run.path.length - 1]!, snap.point)) {
      ...
      if (node) return { system, nodeId: run.endNodeId, point: node.position, createdNodeKind: node.kind };
    }
    ...
    if (firstPath.length < 2 || secondPath.length < 2) {
      return { system, nodeId: fittingNode.id, point: fittingNode.position, createdNodeKind: fittingNode.kind };
    }
  ...
  function addRunEndingAtSegment(params: {
    ...
    draft: PlumbingRunDraft;
    snap: PlumbingRunSegmentSnap;
  }): { system: PlumbingSystem; connected: boolean; fittingKind?: PlumbingNodeKind } {
    const split = splitPlumbingRunAtSegment(params.system, params.snap);
    if (!split) return { system: params.system, connected: false };
    const nextSystem = addRunEndingAtNode({
      system: split.system,
      draft: params.draft,
      endNodeId: split.nodeId,
      endPoint: split.point,
    });
    return { system: nextSystem, connected: true, fittingKind: split.createdNodeKind };
  }
  ...
  function finishPlumbingRunDraft() {
    ...
    if (draft.previewSnap?.kind === 'run-segment') {
      ...
      setSaveState('unsaved');
      setChangedAfterCommit(true);
      setStatus({ tone: 'success', message: draft.system === 'sanitary' ? 'Pipe run finished with a sanitary wye fitting.' : 'Pipe run finished with a pipe fitting.' });
      return true;
    }
  ...
  function handlePlumbingPlanPointer(event: { phase: 'preview' | 'commit'; xMeters: number; zMeters: number; shiftHeld?: boolean }) {
    ...
    if (runSystem) {
      ...
      if (target.kind === 'node' && target.node.id !== plumbingRunDraft.startNodeId && target.node.system === plumbingRunDraft.system) {
        ...
        setSaveState('unsaved');
        setChangedAfterCommit(true);
        setStatus({ tone: 'success', message: 'Pipe run created as a PlumbingRun object.' });
        return;
      }
      if (target.kind === 'run-segment') {
        setPlumbingSystem((current) => {
        ...
        setSaveState('unsaved');
        setChangedAfterCommit(true);
        setStatus({ tone: 'success', message: runSystem === 'sanitary' ? 'Pipe run tied in with a sanitary wye fitting.' : 'Pipe run tied into existing pipe.' });
        return;
      }
      setPlumbingRunDraft((current) =>
        current
