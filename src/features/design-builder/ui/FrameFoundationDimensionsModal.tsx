...
function ModalNumberField({
  ...
  const commit = () => {
    ...
    setDraft(formatInputNumber(typeof min === 'number' ? Math.max(min, parsed) : parsed));
  };
...
export default function FrameFoundationDimensionsModal({
  ...
  const [roofDraft, setRoofDraft] = useState<RoofSystemSettings>(createDefaultRoofSystemSettings());
  const [autoGenerateFrameLayout, setAutoGenerateFrameLayout] = useState(true);

  useEffect(() => {
    if (!isOpen) return;
    setFoundationDraft(normalizeRcFrameFoundationSettings(preset.foundationSettings));
  ...
  const elevations = useMemo(
    () => resolveFoundationElevations({ foundation: foundationDraft, wallHeightMeters }),
    [foundationDraft, wallHeightMeters],
  );

  const foundationValidation = useMemo(
    () => validateRcFrameFoundationSettings({ foundation: foundationDraft, wallHeightMeters }),
    [foundationDraft, wallHeightMeters],
  );
  ...
  const previewFrameSystem = useMemo(
    ...
    [foundationDraft, preset.frameSystem, segmentFrames, wallHeightMeters, wallLayout],
  );
  ...
  const fieldErrors = useMemo(() => {
    ...
    if (foundationDraft.tieBeam.enabled && foundationDraft.tieBeam.widthMeters <= 0) {
      errors.tieWidth = 'Tie Beam width must be greater than zero.';
    }
    if (foundationDraft.isolatedFootings.enabled && foundationDraft.isolatedFootings.widthMeters <= 0) {
    ...
    if (
      ...
      errors.footings = 'Enable frame layout or add structural columns before creating footings.';
    }
    if (roofDraft.enabled && roofDraft.roofAssemblyThicknessMeters <= 0) {
      errors.roofAssemblyThickness = 'Roof assembly thickness must be greater than zero.';
    }
    ...
    if (roofDraft.enabled && roofDraft.purlins.enabled && roofDraft.purlins.maxSpacingMeters <= 0) {
    ...
    if (
      ...
      errors.rakedCapDepth = 'Raked concrete cap depth must be greater than zero when enabled.';
    }
    if (roofDraft.enabled && !resolvedRoof.supported) {
      errors.roofFootprint = resolvedRoof.unsupportedMessage ?? 'Roof generation requires a closed rectangular footprint.';
    }
    if (roofDraft.enabled && roofDraft.peakHeightAboveRoofBeamMeters <= 0) {
      errors.peakHeight = 'Peak height above Roof Beam must be greater than zero.';
    }
    return errors;
  ...
  function patchSteelTrusses(patch: Partial<RoofSystemSettings['steelTrusses']>) {
    setRoofDraft((current) => ({ ...current, steelTrusses: { ...current.steelTrusses, ...patch } }));
  }

  function patchPurlins(patch: Partial<RoofSystemSettings['purlins']>) {
    setRoofDraft((current) => ({ ...current, purlins: { ...current.purlins, ...patch } }));
  }
  ...
  function patchGable(patch: Partial<RoofSystemSettings['gable']>) {
    setRoofDraft((current) => ({ ...current, gable: { ...current.gable, ...patch } }));
  }

  async function handleResetDefaults() {
    const approved = await confirm({
      title: 'Reset frame & foundation defaults',
      message: 'Restore default column, beam, footing, and roof settings for this session?',
          ...
          <div className="space-y-3">
            ...
            >
              ...
              <ModalNumberField
                ...
                onChange={(value) => patchSection('columns', { widthMeters: positiveOrFallback(value, 0.35) })}
              />
              <ModalNumberField
                label="Column Depth"
                ...
                onChange={(value) => patchSection('columns', { depthMeters: positiveOrFallback(value, 0.35) })}
              />
            </CollapsibleSection>
            ...
            >
              <ToggleRow
                ...
                onChange={(checked) => patchSection('plinthBeam', { enabled: checked })}
              />
              <ModalNumberField
                ...
                onChange={(value) => patchSection('plinthBeam', { widthMeters: positiveOrFallback(value, 0.3) })}
              />
              <ModalNumberField
              ...
              <ToggleRow
                ...
                onChange={(checked) => patchSection('plinthBeam', { followsExteriorSegments: checked })}
              />
              <ToggleRow
                label="Apply Along Interior Structural Walls"
                checked={foundationDraft.plinthBeam.followsInteriorSegments}
                onChange={(checked) => patchSection('plinthBeam', { followsInteriorSegments: checked })}
              />
            </CollapsibleSection>
            ...
            >
              <ToggleRow
                ...
                onChange={(checked) => patchSection('tieBeam', { enabled: checked })}
              />
              <ModalNumberField
                ...
                onChange={(value) => patchSection('tieBeam', { widthMeters: positiveOrFallback(value, 0.25) })}
              />
              <ModalNumberField
                label="Tie Beam Depth"
                value={foundationDraft.tieBeam.depthMeters}
                suffix="m"
                onChange={(value) => patchSection('tieBeam', { depthMeters: positiveOrFallback(value, 0.3) })}
              />
            </CollapsibleSection>
            ...
            >
              <ToggleRow
                ...
                onChange={(checked) => patchSection('roofBeam', { enabled: checked })}
              />
              <ModalNumberField
                ...
                onChange={(value) => patchSection('roofBeam', { widthMeters: positiveOrFallback(value, 0.25) })}
              />
              <ModalNumberField
                label="Roof Beam Depth"
                value={foundationDraft.roofBeam.depthMeters}
                suffix="m"
                onChange={(value) => patchSection('roofBeam', { depthMeters: positiveOrFallback(value, 0.3) })}
              />
            </CollapsibleSection>

            <CollapsibleSection id="footings" title="Isolated Footings" helper="Footing Drop Below Plinth Beam is measured from the bottom of the Plinth Beam down to the top of the isolated footing. Tie Beams rest directly on footing tops.">
              <ToggleRow
                label="Create Footings at Structural Columns"
                checked={foundationDraft.isolatedFootings.autoCreateAtStructuralColumns}
                onChange={(checked) =>
              ...
              <ModalNumberField
                ...
                suffix="m"
                onChange={(value) =>
                  patchSection('isolatedFootings', { lengthMeters: positiveOrFallback(value, 1.2) })
                }
              />
              <ModalNumberField
                ...
                suffix="m"
                onChange={(value) =>
                  patchSection('isolatedFootings', { thicknessMeters: positiveOrFallback(value, 0.45) })
                }
              />
              <ModalNumberField
            ...
            >
              ...
              <ModalNumberField
                ...
                onChange={(value) => patchRoof({ peakHeightAboveRoofBeamMeters: positiveOrFallback(value, 1.25) })}
              />
              <ModalNumberField
                label="Eave Overhang"
              ...
              <label className="block text-sm">
                ...
                >
                  ...
                  <option value="along_selected_wall_pair">Select Wall Pair</option>
                </select>
              </label>
              ...
              {roofDraft.roofType === 'gable' ? (
                <>
                  ...
                  <ModalNumberField
                    ...
                    suffix="×"
                    onChange={(value) => patchSteelTrusses({ webSteelAllowanceFactor: positiveOrFallback(value, 0.35) })}
                  />
                  <ToggleRow
                    label="Base Plates Enabled"
                    checked={roofDraft.steelTrusses.basePlateEnabled}
                    onChange={(checked) => patchSteelTrusses({ basePlateEnabled: checked })}
                  />
                  <ModalNumberField
                    label="Base Plate Width"
                    value={roofDraft.steelTrusses.basePlateWidthMeters}
                    suffix="m"
                    onChange={(value) => patchSteelTrusses({ basePlateWidthMeters: positiveOrFallback(value, 0.2) })}
                  />
                  <ModalNumberField
                    label="Base Plate Length"
                    value={roofDraft.steelTrusses.basePlateLengthMeters}
                    suffix="m"
                    onChange={(value) => patchSteelTrusses({ basePlateLengthMeters: positiveOrFallback(value, 0.25) })}
                  />
                  <ModalNumberField
                    label="Base Plate Thickness"
                    value={roofDraft.steelTrusses.basePlateThicknessMeters}
                    suffix="m"
                    onChange={(value) => patchSteelTrusses({ basePlateThicknessMeters: positiveOrFallback(value, 0.012) })}
                  />
                  <ModalNumberField
                    ...
                    min={1}
                    onChange={(value) =>
                      patchSteelTrusses({ anchorBoltsPerBearing: Math.max(1, Math.round(positiveOrFallback(value, 4))) })
                    }
                  />
                </>
              ) : null}
              ...
              <ModalNumberField
                ...
                suffix="%"
                onChange={(value) => patchCorrugatedMetal({ wastePercent: positiveOrFallback(value, 10) })}
              />
              <ToggleRow
                label="Ridge Cap Enabled"
                checked={roofDraft.corrugatedMetal.ridgeCapEnabled}
                onChange={(checked) => patchCorrugatedMetal({ ridgeCapEnabled: checked })}
              />
              <ModalNumberField
