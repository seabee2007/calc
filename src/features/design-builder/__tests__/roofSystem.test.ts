...
import {
  buildFrameInfillEstimatePreview,
...
function resolveRoofFromPreset(
  ...
  const segmentFrames = getSegmentFramesForWallLayout(
    preset.wallLayout,
    preset.wall,
  );
...
function splitLongRectangleLayout(
  lengthMeters: number,
  widthMeters: number,
...
function pointInTrianglePlan(
  ...
  const sign = (
    p1: Pick<RoofVec3, "x" | "z">,
    p2: Pick<RoofVec3, "x" | "z">,
    p3: Pick<RoofVec3, "x" | "z">,
...
function pointInPlanePlan(
  ...
  if (d) {
    return (
      pointInTrianglePlan(point, a!, b!, c!) ||
      pointInTrianglePlan(point, a!, c!, d)
    );
  }
...
function planCentroid(plane: RoofPlane): Pick<RoofVec3, "x" | "z"> {
  return {
    x:
      plane.corners.reduce((sum, corner) => sum + corner.x, 0) /
      plane.corners.length,
    z:
      plane.corners.reduce((sum, corner) => sum + corner.z, 0) /
      plane.corners.length,
  };
}
...
describe("Roof system — hip, gable, raked cap", () => {
  ...
  it("defaults rake clearance to 0.1016 m (4 in)", () => {
    expect(createDefaultRoofSystemSettings().gable.rakeClearanceMeters).toBe(
      0.1016,
    );
  });
  ...
  it("defaults fascia trim off with a one-inch frame drop", () => {
    ...
    expect(defaults.fascia.bottomExtensionBelowFrameMeters).toBeCloseTo(
      0.0254,
      6,
    );
  ...
  it("resolves boxed soffit panels around gable overhangs by default", () => {
    ...
    const sideEaves = roof.soffitPlacements.filter(
      (placement) => placement.edgeRole === "side_eave",
    );
    const gableReturns = roof.soffitPlacements.filter(
      (placement) => placement.edgeRole === "gable_return",
    );
    ...
    for (const placement of sideEaves) {
      ...
      expect(
        Math.abs(placement.innerStart.y - placement.outerStart.y),
  ...
  it("resolves boxed soffit panels around hip eaves", () => {
    ...
    expect(roof.soffitPlacements.length).toBeGreaterThanOrEqual(4);
    expect(
      roof.soffitPlacements.every(
        (placement) => placement.edgeRole === "hip_eave",
      ),
    ).toBe(true);
    expect(
      totalRoofSoffitAreaSquareMeters(roof.soffitPlacements),
    ).toBeGreaterThan(0);
  });
  ...
  it("rectangular hip roof creates a centered long-axis ridge and four valid planes", () => {
    ...
    expect(
      roof.roofTopPlanes.filter((plane) => plane.corners.length === 4),
    ).toHaveLength(2);
    expect(
      roof.roofTopPlanes.filter((plane) => plane.corners.length === 3),
    ).toHaveLength(2);
    ...
    expect(roof.roofSurfaceAreaSquareMeters).toBeCloseTo(
      10 * 6 * Math.sqrt(1 + 0.5 ** 2),
      3,
    );
    ...
    for (const plane of roof.roofTopPlanes) {
      ...
      const containingPlanes = roof.roofTopPlanes.filter((candidate) =>
        pointInPlanePlan(centroid, candidate),
      );
      expect(containingPlanes.map((candidate) => candidate.id)).toEqual([
        plane.id,
      ]);
    }
    ...
    for (const corner of roof.claddingPerimeter) {
      ...
      const eaveMembershipCount = roof.roofTopPlanes.filter((plane) =>
        plane.corners.some((planeCorner) =>
          pointMatches(planeCorner, eaveCorner),
        ),
      ).length;
      ...
      expect(
        roof.roofTopPlanes.some(
          (plane) =>
            ...
            plane.corners.some((planeCorner) =>
              pointMatches(planeCorner, nearestRidgeEnd),
            ),
        ),
      ).toBe(true);
    }
    for (const ridgeEnd of ridgeEnds) {
      const ridgeMembershipCount = roof.roofTopPlanes.filter((plane) =>
        plane.corners.some((planeCorner) =>
          pointMatches(planeCorner, ridgeEnd),
        ),
      ).length;
  ...
  it("generates steel trusses across the full length of buildings longer than 10m", () => {
    ...
    expect(roof.trussPlacements.at(-1)?.stationMeters).toBeCloseTo(
      roof.trussStations.at(-1)!,
      6,
    );
  });
  ...
  it("hip roof on a square creates a pyramid roof with one peak", () => {
    ...
    expect(roof?.roofTopPlanes.length).toBe(4);
    expect(
      roof?.roofTopPlanes.every((plane) => plane.corners.length === 3),
    ).toBe(true);
  });
      ...
      expect(
        roof.roofTopPlanes.some(
          (plane) =>
            ...
            plane.corners.some((planeCorner) =>
              pointMatches(planeCorner, roof.peakPoint!),
            ),
        ),
      ).toBe(true);
    }
  ...
  it("hip eave overhang lowers cladding eaves without moving structural bearing or peak", () => {
    ...
    const noOverhangEaveY = Math.min(
      ...noOverhang.roofTopPlanes.flatMap((plane) =>
        plane.corners.map((corner) => corner.y),
      ),
    );
    const overhangEaveY = Math.min(
      ...withOverhang.roofTopPlanes.flatMap((plane) =>
        plane.corners.map((corner) => corner.y),
      ),
    );
    ...
    expect(withOverhang.structuralRidgeStart).toEqual(
      noOverhang.structuralRidgeStart,
    );
    expect(withOverhang.structuralRidgeEnd).toEqual(
      noOverhang.structuralRidgeEnd,
    );
    expectPointClose(
      withOverhang.claddingRidgeStart!,
      withOverhang.structuralRidgeStart!,
      3,
    );
    expectPointClose(
      withOverhang.claddingRidgeEnd!,
      withOverhang.structuralRidgeEnd!,
      3,
    );
  ...
  it("ridge direction changes the correct gable-end segments", () => {
    ...
    const alongLongest = resolveRoofFromPreset(
      preset,
      {
        ...createDefaultRoofSystemSettings(),
        roofType: "gable",
    ...
    expect(alongLongest.gableEndSegmentIds).not.toEqual(
      alongShortest.gableEndSegmentIds,
    );
  });
  ...
  it("gable ridge spans bearing perimeter with eaves at cladding overhang", () => {
    ...
    const eavePlane = roof.roofTopPlanes.find(
      (plane) => plane.corners.length === 4,
    );
    ...
    const eaveCorners = eavePlane!.corners.filter(
      (corner) => Math.abs(corner.y - eaveY) < 0.001,
    );
  ...
  it("eave overhang expands roof footprint without changing wall coordinates", () => {
    ...
    expect(
      withOverhang.claddingRafterRunMeters - noOverhang.claddingRafterRunMeters,
    ).toBeCloseTo(0.5, 3);
    expect(withOverhang.roofPeakY).toBeCloseTo(noOverhang.roofPeakY, 6);
    expect(
      Math.atan2(
        withOverhang.rafterRiseMeters,
        withOverhang.structuralRafterRunMeters,
      ),
    ).toBeCloseTo(
      Math.atan2(
        noOverhang.rafterRiseMeters,
        noOverhang.structuralRafterRunMeters,
      ),
      6,
    );
  ...
  it("peak height updates roof rise and member reference length", () => {
    ...
    expect(highPeak.rafterLengthMeters).toBeGreaterThan(
      lowPeak.rafterLengthMeters,
    );
  ...
  it("gable CMU remains below roof underside minus configured rake clearance", () => {
    ...
    const frameById = new Map(
      segmentFrames.map((frame) => [frame.segmentId, frame]),
    );
    for (const gableEnd of roof.gableEnds) {
      ...
      for (const block of gableEnd.cmuUnitPlacements) {
        ...
        const endStation =
          block.endAlongMeters ?? startStation + block.lengthMeters;
        const clearanceStart = roofClearanceElevationAtStation({
          resolvedRoof: roof,
          frame: frame!,
          stationMeters: startStation,
          panelStartStation:
            gableEnd.masonryCourses[0]?.startStationMeters ?? 0,
          panelEndStation:
            gableEnd.masonryCourses[0]?.endStationMeters ?? frame!.lengthMeters,
        ...
        expect(unitTop).toBeLessThanOrEqual(
          allowedTop + GABLE_HEIGHT_TOLERANCE_METERS + 0.01,
        );
      }
    }
  });
  ...
  it("gable masonry keeps individual CMU units with running-bond offsets", () => {
    const geometry = frameInfillGeometry(gableRoofSystem());
    const blocks =
      geometry.resolvedRoofSystem?.gableEnds.flatMap(
        (end) => end.cmuUnitPlacements,
      ) ?? [];
    expect(blocks.length).toBeGreaterThan(2);
    const courseIndices = [
      ...new Set(blocks.map((block) => block.courseIndex ?? 0)),
    ].sort((a, b) => a - b);
    const firstCourse = blocks.filter(
      (block) => block.courseIndex === courseIndices[0],
    );
    const secondCourse = blocks.filter(
      (block) => block.courseIndex === courseIndices[1],
    );
  ...
  it("gable courses use full units before half or cut units when possible", () => {
    ...
    const lowestCourseIndex = Math.min(
      ...blocks.map((block) => block.courseIndex ?? 0),
    );
    const bottomCourse = blocks.filter(
      (block) => block.courseIndex === lowestCourseIndex,
    );
  ...
  it("raked cap fills resolved stair-to-roof space and produces concrete volume", () => {
    ...
    expect(caps.every((cap) => cap.source === "gable_raked_concrete_cap")).toBe(
      true,
    );
    ...
    expect(
      geometry.resolvedRoofSystem?.rakedCapVolumeCubicMeters ?? 0,
    ).toBeGreaterThan(0);
  });
  ...
  it("raked cap segments maintain minimum configured depth and positive volume", () => {
    ...
    for (const cap of caps) {
      ...
      expect(cap.startTopY - cap.startBottomY).toBeGreaterThanOrEqual(
        minDepth - GABLE_HEIGHT_TOLERANCE_METERS,
      );
      expect(cap.endTopY - cap.endBottomY).toBeGreaterThanOrEqual(
        minDepth - GABLE_HEIGHT_TOLERANCE_METERS,
      );
    }
    expect(minimumRakedCapDepthMeters(caps)).toBeGreaterThanOrEqual(
      minDepth - GABLE_HEIGHT_TOLERANCE_METERS,
    );
  });
  ...
  it("changing peak height recalculates gable CMU and cap geometry", () => {
    ...
    expect(
      high.resolvedRoofSystem?.rakedCapVolumeCubicMeters ?? 0,
    ).toBeGreaterThan(low.resolvedRoofSystem?.rakedCapVolumeCubicMeters ?? 0);
    expect(
      (
        high.resolvedRoofSystem?.gableEnds.flatMap(
          (end) => end.cmuUnitPlacements,
        ) ?? []
      ).length,
    ).toBeGreaterThan(
      (
        low.resolvedRoofSystem?.gableEnds.flatMap(
          (end) => end.cmuUnitPlacements,
        ) ?? []
      ).length,
    );
    ...
    const loose = frameInfillGeometry(
      gableRoofSystem({
        gable: {
          ...createDefaultRoofSystemSettings().gable,
          enabled: true,
          rakeClearanceMeters: 0.2,
          rakedConcreteCapEnabled: true,
        },
      }),
    );
    expect(
      loose.resolvedRoofSystem?.rakedCapVolumeCubicMeters ?? 0,
    ).toBeGreaterThan(tight.resolvedRoofSystem?.rakedCapVolumeCubicMeters ?? 0);
  });
  ...
  it("saving and reloading preserves gable cap settings", () => {
    ...
    updated.roofSystem = gableRoofSystem({
      peakHeightAboveRoofBeamMeters: 1.6,
    });
  ...
  it("raked cap is separate from grout quantities", () => {
    ...
    const groutLines = preview.filter((line) =>
      line.description.toLowerCase().includes("grout"),
    );
    expect(capLine).toBeDefined();
    expect(capLine?.quantityType).toBe("raked_concrete_cap_volume");
    expect(
      groutLines.every(
        (line) => line.quantityType !== "raked_concrete_cap_volume",
      ),
    ).toBe(true);
  });
  ...
  it("raked cap estimate volume matches resolved segment geometry", () => {
    const geometry = frameInfillGeometry(gableRoofSystem());
    const segmentVolume = totalRakedCapVolumeCubicMeters(
      geometry.rakedCapPlacements ?? [],
    );
    expect(
      geometry.resolvedRoofSystem?.rakedCapVolumeCubicMeters ?? 0,
    ).toBeCloseTo(segmentVolume, 6);
    const preview = buildFrameInfillEstimatePreview({
      designModelId: "test-model",
      wallObjectId: "wall",
      slabObjectId: "slab",
    ...
    expect(capLine?.parameterSnapshot.rakedCapVolumeCubicMeters).toBeCloseTo(
      segmentVolume,
      6,
    );
    expect(capLine?.quantity).toBeCloseTo(
      cubicMetersToCubicYards(segmentVolume),
      2,
    );
  });
  ...
  it("adds fascia trim as a separate roof linear quantity when enabled", () => {
    ...
    const geometry = frameInfillGeometry(roofSystem);
    const fasciaMeters = totalRoofFasciaLengthMeters(
      geometry.resolvedRoofSystem?.fasciaPlacements ?? [],
    );
    const preview = buildFrameInfillEstimatePreview({
      designModelId: "test-model",
      wallObjectId: "wall",
    ...
    expect(fasciaLine?.parameterSnapshot.fasciaLinearMeters).toBeCloseTo(
      fasciaMeters,
      6,
    );
  });
  ...
  it("adds soffit panels as a separate roof area quantity when enabled", () => {
    const roofSystem = gableRoofSystem();
    const geometry = frameInfillGeometry(roofSystem);
    const soffitSquareMeters = totalRoofSoffitAreaSquareMeters(
      geometry.resolvedRoofSystem?.soffitPlacements ?? [],
    );
    const preview = buildFrameInfillEstimatePreview({
      designModelId: "test-model",
      wallObjectId: "wall",
    ...
    expect(soffitLine?.quantity).toBeCloseTo(
      squareMetersToSquareFeet(soffitSquareMeters),
      2,
    );
    expect(soffitLine?.parameterSnapshot.soffitAreaSquareMeters).toBeCloseTo(
      soffitSquareMeters,
      6,
    );
  });
  ...
  it("omits soffit panel quantity when soffit is disabled", () => {
    ...
    expect(preview.some((line) => line.id === "roof-soffit-panels")).toBe(
      false,
    );
  });
  ...
  it("unsupported footprints show warning instead of invalid geometry", () => {
    ...
    expect(
      roof.warnings.some(
        (warning) => warning.message === UNSUPPORTED_ROOF_FOOTPRINT_MESSAGE,
      ),
    ).toBe(true);
...
describe("Gable-end / rake overhang", () => {
  ...
  function purlinPlanLength(purlin: {
  ...
  function ridgeRowPurlinLength(
    roof: ReturnType<typeof gableRoofWithOverhang>,
  ) {
    const ridgeRow = roof.purlinRowsPerSlope - 1;
    const purlin = roof.purlinPlacements.find(
      (entry) => entry.rowIndex === ridgeRow,
    );
  ...
  it("zero gable-end overhang ends purlins at structural gable faces", () => {
    ...
    expect(roof.structuralRidgeLengthMeters).toBeCloseTo(
      roof.claddingRidgeLengthMeters,
      3,
    );
    expect(ridgeRowPurlinLength(roof)).toBeCloseTo(
  ...
  it("positive gable-end overhang extends cladding ridge equally beyond both structural gable ends", () => {
    ...
    expect(roof.claddingRidgeLengthMeters).toBeCloseTo(
      roof.structuralRidgeLengthMeters + overhang * 2,
      3,
    );
  });
  ...
  it("purlins span the complete cladding length", () => {
    ...
    const ridgeRow = roof.purlinRowsPerSlope - 1;
    for (const purlin of roof.purlinPlacements.filter(
      (entry) => entry.rowIndex === ridgeRow,
    )) {
      expect(purlinPlanLength(purlin)).toBeCloseTo(
        roof.claddingRidgeLengthMeters,
        2,
      );
    }
  ...
  it("roof sheets span the complete cladding length", () => {
    ...
    for (const corner of ridgeCorners) {
      const nearCladdingStart =
        Math.hypot(
          corner.x - roof.claddingRidgeStart!.x,
          corner.z - roof.claddingRidgeStart!.z,
        ) < 0.01;
      const nearCladdingEnd =
        Math.hypot(
          corner.x - roof.claddingRidgeEnd!.x,
          corner.z - roof.claddingRidgeEnd!.z,
        ) < 0.01;
      expect(nearCladdingStart || nearCladdingEnd).toBe(true);
    }
    expect(
      ridgeLengthMeters(ridgeCorners[0]!, ridgeCorners[1] ?? ridgeCorners[0]!),
    ).toBeGreaterThan(0);
  });
  ...
  it("gable side-eave fascia sits plumb on the outside face of the eave purlin", () => {
    ...
    const supportMaxX = Math.max(
      ...roof.claddingPerimeter.map((point) => Math.abs(point.x)),
    );
    const supportMaxZ = Math.max(
      ...roof.claddingPerimeter.map((point) => Math.abs(point.z)),
    );
    const sheetMaxX = Math.max(
      ...roof.roofSheetPerimeter.map((point) => Math.abs(point.x)),
    );
    const sheetMaxZ = Math.max(
      ...roof.roofSheetPerimeter.map((point) => Math.abs(point.z)),
    );
    ...
    const sideEaves = roof.fasciaPlacements.filter(
      (placement) => placement.edgeRole === "side_eave",
    );
    const gableRakes = roof.fasciaPlacements.filter(
      (placement) => placement.edgeRole === "gable_rake",
    );

    expect(sheetMaxX - supportMaxX).toBeCloseTo(
      ROOF_SHEET_EAVE_OVERHANG_METERS,
      3,
    );
    expect(sheetMaxZ - supportMaxZ).toBeCloseTo(
      ROOF_SHEET_EAVE_OVERHANG_METERS,
      3,
    );
    expect(fasciaMaxX).toBeCloseTo(supportMaxX, 2);
    expect(sheetMaxX - fasciaMaxX).toBeCloseTo(
      ROOF_SHEET_EAVE_OVERHANG_METERS,
      2,
    );
    ...
    for (const fascia of sideEaves) {
      const purlin = roof.purlinPlacements.find(
        (placement) =>
          placement.slopePlaneId === fascia.sourcePlaneId &&
          placement.rowIndex === 0,
      );
      expect(purlin).toBeDefined();
      const project = (point: { x: number; z: number }) =>
        point.x * fascia.faceOutwardNormal.x +
        point.z * fascia.faceOutwardNormal.z;
      const purlinFaceProjection =
        Math.max(project(purlin!.start), project(purlin!.end)) +
      ...
      for (const soffit of roof.soffitPlacements.filter(
        (placement) => placement.edgeRole === "side_eave",
      )) {
        const soffitOuterProjection = Math.max(
          project(soffit.outerStart),
          project(soffit.outerEnd),
        );
        expect(soffitOuterProjection).toBeLessThanOrEqual(
          purlinFaceProjection + 0.001,
        );
      }
    }
  ...
  it("gable CMU stops at structural gable faces", () => {
    ...
    const roof = geometry.resolvedRoofSystem!;
    const segmentFrames = geometry.wallCmuLayout.segmentFrames ?? [];
    for (const block of geometry.blockInstances.filter(
      (entry) => entry.source === "gable_end_solver",
    )) {
      const gableEnd = roof.gableEnds.find(
        (entry) => entry.hostSegmentId === block.segmentId,
      );
      const firstCourse = gableEnd?.masonryCourses[0];
      const frame = segmentFrames.find(
        (entry) => entry.segmentId === gableEnd?.hostSegmentId,
      );
      if (!gableEnd || !firstCourse || !frame) continue;
      const station = projectPointToSegmentStation(
        { x: block.x, z: block.z },
        frame,
      );
      expect(station).toBeGreaterThanOrEqual(
        firstCourse.startStationMeters - 0.01,
      );
      expect(station).toBeLessThanOrEqual(firstCourse.endStationMeters + 0.01);
    }
  ...
  it("raked concrete cap stops at structural gable faces", () => {
    ...
    for (const cap of geometry.rakedCapPlacements ?? []) {
      const hostPanel = preset.infillSystem.panels.find(
        (entry) => entry.hostSegmentId === cap.gableEndSegmentId,
      );
      if (!hostPanel) continue;
      expect(cap.startStationMeters).toBeGreaterThanOrEqual(
        hostPanel.startStationMeters - 0.01,
      );
      expect(cap.endStationMeters).toBeLessThanOrEqual(
        hostPanel.endStationMeters + 0.01,
      );
    }
  });
  ...
  it("ridge cap remains a thin folded cover above roof sheets", () => {
    ...
    expect(roof.ridgeCapPlacement!.start.y).toBeCloseTo(
      roof.ridgeCapPlacement!.end.y,
      3,
    );
  ...
  it("fascia trim follows gable eaves and rakes below structural frame ends", () => {
    ...
    for (const sideEave of sideEaves) {
      expect(sideEave.lengthMeters).toBeCloseTo(
        roof.claddingRidgeLengthMeters,
        2,
      );
    }
    ...
    for (const placement of roof.fasciaPlacements) {
      const expectedFrameDepth =
        frameStackAlongRoofNormal * Math.max(0, placement.outwardNormal.y);
      expect(placement.faceDepthMeters).toBeCloseTo(
        expectedFrameDepth +
          roofSystem.fascia.bottomExtensionBelowFrameMeters +
          soffitFasciaLapMeters,
        6,
      );
      expect(placement.topStart.y - placement.bottomStart.y).toBeCloseTo(
        placement.faceDepthMeters,
        6,
      );
      expect(placement.topEnd.y - placement.bottomEnd.y).toBeCloseTo(
        placement.faceDepthMeters,
        6,
      );
      expect(placement.bottomStart.y).toBeLessThan(
        placement.topStart.y - expectedFrameDepth,
      );
      expect(placement.bottomEnd.y).toBeLessThan(
        placement.topEnd.y - expectedFrameDepth,
      );
    }
  });
  ...
  it("rotated ridge direction resolves the same overhang behavior", () => {
    ...
    expect(
      alongShortest.claddingRidgeLengthMeters -
        alongShortest.structuralRidgeLengthMeters,
    ).toBeCloseTo(1, 3);
    expect(
      alongLongest.claddingRidgeLengthMeters -
        alongLongest.structuralRidgeLengthMeters,
    ).toBeCloseTo(1, 3);
    expect(alongShortest.purlinPlacements[0]).toBeDefined();
    expect(ridgeRowPurlinLength(alongShortest)).toBeCloseTo(
      alongShortest.claddingRidgeLengthMeters,
      2,
    );
  });
