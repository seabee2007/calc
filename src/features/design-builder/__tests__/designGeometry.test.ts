...
describe("Design Builder generated geometry", () => {
  ...
  it("generates CMU block instances from wall parameters without storing per-block source objects", () => {
    ...
    expect(instances.some((instance) => instance.id.startsWith("south-"))).toBe(
      true,
    );
    expect(instances.some((instance) => instance.id.startsWith("east-"))).toBe(
      true,
    );
  });
  ...
  it("does not generate block instances inside door and window openings", () => {
    ...
    const doorOpening = layout.roughOpenings.find(
      (opening) => opening.id === "door-west-01",
    )!;
    const windowOpening = layout.roughOpenings.find(
      (opening) => opening.id === "window-east-01",
    )!;
    ...
    const doorBlocks = instances.filter(
      (instance) =>
        ...
        instance.x >=
          -preset.wall.lengthMeters / 2 + doorOpening.actualStartAlongMeters &&
        instance.x <=
          -preset.wall.lengthMeters / 2 + doorOpening.actualEndAlongMeters &&
    ...
    const windowBlocks = instances.filter(
      (instance) =>
        ...
        instance.z >=
          -preset.wall.widthMeters / 2 + windowOpening.actualStartAlongMeters &&
        instance.z <=
          -preset.wall.widthMeters / 2 + windowOpening.actualEndAlongMeters &&
  ...
  it("generates CMU courses and running bond special units", () => {
    ...
    expect(new Set(layout.blocks.map((block) => block.course)).size).toBe(
      layout.courseCount,
    );
    expect(layout.counts.full).toBeGreaterThan(0);
    expect(
      layout.counts.half +
        layout.counts.corner +
        layout.counts.jamb +
        layout.counts.cut,
    ).toBeGreaterThan(0);
  });
  ...
  it("generates lintels above openings with default bearing", () => {
    ...
    expect(layout.lintels[0].lengthMeters).toBeCloseTo(
      layout.roughOpenings[0].lintelLengthMeters,
      6,
    );
  ...
  it("running bond alternates courses by a half module", () => {
    ...
    const courseZero = layout.blocks.find(
      (block) => block.face === "north" && block.course === 0,
    );
    const courseOne = layout.blocks.find(
      (block) => block.face === "north" && block.course === 1,
    );

    expect(courseZero?.blockType).not.toBe("half");
    expect(courseOne?.blockType).toBe("half");
    expect(courseOne?.lengthMeters).toBeCloseTo(
      (preset.wall.blockModule?.actualLengthMeters ?? 0.39) / 2,
      6,
    );
  });
  ...
  it("places CMU courses upward on Y by module height and bed joint", () => {
    ...
    expect((courseOne?.y ?? 0) - (courseZero?.y ?? 0)).toBeCloseTo(
      preset.wall.blockModule?.moduleHeightMeters ?? 0.2,
      6,
    );
  });
  ...
  it("running bond and stack bond use distinct station offsets", () => {
    const preset = createFiveBySixCmuBuildingPreset();
    const running = generateCmuLayout({
      ...preset.wall,
      openings: [],
      bondPattern: "running_bond",
    });
    const stack = generateCmuLayout({
      ...preset.wall,
      openings: [],
      bondPattern: "stack_bond",
    });
    const runningCourseZero = running.blocks.find(
      (block) => block.face === "north" && block.course === 0,
    );
    const runningCourseOne = running.blocks.find(
      (block) => block.face === "north" && block.course === 1,
    );
    const stackCourseZero = stack.blocks.find(
      (block) => block.face === "north" && block.course === 0,
    );
    const stackCourseOne = stack.blocks.find(
      (block) => block.face === "north" && block.course === 1,
    );

    expect(runningCourseZero?.startAlongMeters).toBeCloseTo(
      stackCourseZero?.startAlongMeters ?? -1,
      6,
    );
    expect(runningCourseOne?.lengthMeters).toBeCloseTo(
      (preset.wall.blockModule?.actualLengthMeters ?? 0.39) / 2,
      6,
    );
    expect(stackCourseZero?.startAlongMeters).toBeCloseTo(
      stackCourseOne?.startAlongMeters ?? -1,
      6,
    );
    expect(stackCourseZero?.lengthMeters).toBeCloseTo(
      stackCourseOne?.lengthMeters ?? -1,
      6,
    );
  });
  ...
  it("generates 3D wall geometry from the wall layout graph when present", () => {
    ...
    expect(geometry.wallSegments).toHaveLength(
    ...
    expect(geometry.blockInstances).toHaveLength(
      geometry.wallCmuLayout.blocks.length,
    );
    expect(
      geometry.wallCmuLayout.roughOpenings.every(
        (opening) => "worldX" in opening && "worldZ" in opening,
      ),
    ).toBe(true);
  });
  ...
  it("layout graph path derives rendered blocks from layout wallCmuLayout only", () => {
    ...
    expect(geometry.sourcePath).toBe("layout_graph");
    expect(geometry.wallCmuLayout.blocks.map((block) => block.id)).toEqual(
      geometry.blockInstances.map((block) => block.id),
    );
    expect(
      geometry.wallCmuLayout.blocks.every((block) => block.segmentId),
    ).toBe(true);
  ...
  it("derived placement list accounts for every rendered CMU unit", () => {
    ...
    expect(geometry.wallCmuLayout.unitPlacements).toHaveLength(
      geometry.blockInstances.length,
    );
    expect(
      geometry.wallCmuLayout.unitPlacements.map((placement) => placement.id),
    ).toEqual(geometry.blockInstances.map((block) => block.id));
    expect(
      geometry.wallCmuLayout.unitPlacements.every(
        (placement) =>
  ...
  it("layout-generated opening, lintel, and jamb geometry comes from wallSegmentId", () => {
    ...
    const windowOpening = geometry.wallCmuLayout.roughOpenings.find(
      (opening) => opening.id === "window-east-01",
    ) as
      ...
      | undefined;
    const doorOpening = geometry.wallCmuLayout.roughOpenings.find(
      (opening) => opening.id === "door-west-01",
    );
    const windowLintel = geometry.wallCmuLayout.lintels.find(
      (lintel) => lintel.openingId === "window-east-01",
    );
    const doorLintel = geometry.wallCmuLayout.lintels.find(
      (lintel) => lintel.openingId === "door-west-01",
    );
    const windowGrout = geometry.wallCmuLayout.jambGroutCells.filter(
      (cell) => cell.openingId === "window-east-01",
    );
    const moduleConfig = resolveCmuModuleConfig(preset.wall);

    expect(windowOpening?.wallSegmentId).toBe(
      preset.wall.openings.find((opening) => opening.id === "window-east-01")
        ?.wallSegmentId,
    );
    ...
    expect(windowLintel?.heightMeters).toBeCloseTo(
      moduleConfig.actualHeightMeters,
      6,
    );
    expect(windowLintel?.depthMeters).toBeCloseTo(
      preset.wall.wallThicknessMeters,
      6,
    );
    expect(windowLintel?.lengthMeters).toBeCloseTo(
      (windowOpening?.actualWidthMeters ?? 0) +
        (windowOpening?.lintelBearingMeters ?? 0) * 2,
      6,
    );
    expect(windowLintel?.courseIndex).toBe(
      Math.ceil(
        (windowOpening?.roughTopMeters ?? 0) / moduleConfig.moduleHeightMeters,
      ),
    );
    expect(windowLintel?.y).toBeCloseTo(
      (windowLintel?.courseIndex ?? 0) * moduleConfig.moduleHeightMeters +
        moduleConfig.actualHeightMeters / 2,
      6,
    );
    expect(windowOpening?.actualTopMeters).toBeCloseTo(
      doorOpening?.actualTopMeters ?? 0,
      6,
    );
    ...
    expect(
      windowGrout.every(
        (cell) => cell.segmentId === windowOpening?.wallSegmentId,
      ),
    ).toBe(true);
  });
  ...
  it("layout-graph door void and frame share the same segment station frame", () => {
    ...
    const voidBlocks = geometry.blockInstances.filter((block) => {
      ...
      const inActual =
        end > (door?.actualStartAlongMeters ?? 0) &&
        start < (door?.actualEndAlongMeters ?? 0);
      const courseBottom = (block.courseIndex ?? block.course) * moduleHeight;
      const courseTop = courseBottom + moduleHeight;
      const inHeight =
        courseBottom < (door?.actualTopMeters ?? 0) &&
        courseTop > (door?.actualBottomMeters ?? 0);
    ...
    const nearestVoidEdge = geometry.blockInstances
      ...
      .map((block) =>
        Math.min(
          Math.abs(
            (block.stationMeters ?? 0) - (door?.actualStartAlongMeters ?? 0),
          ),
          Math.abs(
            (block.stationMeters ?? 0) +
              (block.actualLengthMeters ?? block.lengthMeters) -
              (door?.actualEndAlongMeters ?? 0),
          ),
        ),
      )
  ...
  it("moving a wall segment moves its attached opening and lintel with the segment frame", () => {
    const preset = createFiveBySixCmuBuildingPreset();
    const windowSegmentId = preset.wall.openings.find(
      (opening) => opening.id === "window-east-01",
    )?.wallSegmentId;
    const windowSegment = preset.wallLayout.segments.find(
      (segment) => segment.id === windowSegmentId,
    )!;
    const movedOnce = moveWallNode(
      preset.wallLayout,
      windowSegment.startNodeId,
      3.4,
      -2.5,
    );
    const movedLayout = moveWallNode(
      movedOnce,
      windowSegment.endNodeId,
      3.4,
      2.5,
    );
    ...
    const baseWindow = base.wallCmuLayout.roughOpenings.find(
      (opening) => opening.id === "window-east-01",
    ) as
      | ((typeof base.wallCmuLayout.roughOpenings)[number] & {
          worldX?: number;
          worldZ?: number;
        })
      | undefined;
    const movedWindow = moved.wallCmuLayout.roughOpenings.find(
      (opening) => opening.id === "window-east-01",
    ) as
      | ((typeof moved.wallCmuLayout.roughOpenings)[number] & {
          worldX?: number;
          worldZ?: number;
        })
      | undefined;
    const movedLintel = moved.wallCmuLayout.lintels.find(
      (lintel) => lintel.openingId === "window-east-01",
    );
  ...
  it("newly drawn layout segments appear in generated 3D wall geometry", () => {
    ...
    const drawnLayout = addWallSegment(
      preset.wallLayout,
      startNodeId,
      -3,
      -3.4,
    ...
    const firstNonZeroStart = (geometry: typeof running, course: number) =>
      geometry.blockInstances
        .filter(
          (block) =>
            block.course === course &&
            (block.stationMeters ?? 0) > moduleLength / 4,
        )
        .sort((a, b) => (a.stationMeters ?? 0) - (b.stationMeters ?? 0))[0]
        ?.stationMeters;
  ...
  it("closed rectangle uses one global running-bond phase instead of per-wall restarts", () => {
    ...
    const courseZeroCorners =
      geometry.wallCmuLayout.cornerAssemblies?.filter(
        (assembly) => assembly.courseIndex === 0,
      ) ?? [];
    const courseOneCorners =
      geometry.wallCmuLayout.cornerAssemblies?.filter(
        (assembly) => assembly.courseIndex === 1,
      ) ?? [];
    ...
    expect(new Set(courseZeroCorners.map((corner) => corner.phase))).toEqual(
      new Set([0]),
    );
    expect(new Set(courseOneCorners.map((corner) => corner.phase))).toEqual(
      new Set([1]),
    );
    expect(courseZeroStarts).toHaveLength(4);
    expect(courseOneStarts).toHaveLength(0);
    expect(firstCourseStarts.filter((station) => station <= 0.01)).toHaveLength(
      4,
    );
  });
  ...
  it("closed rectangle creates outside corner weave layouts that alternate ownership", () => {
    ...
    const courseZero = geometry.cornerCourseLayouts.filter(
      (corner) => corner.courseIndex === 0,
    );
    const courseOne = geometry.cornerCourseLayouts.filter(
      (corner) => corner.courseIndex === 1,
    );
    ...
    expect(courseZero.every((corner) => corner.cornerType === "outside")).toBe(
      true,
    );
    expect(
      courseZero.every(
        (corner) => corner.strategy === "interlocked_running_bond",
      ),
    ).toBe(true);
  ...
  it("layout corner weave generates corner and end units without duplicate corner blocks", () => {
    ...
    const firstCourseBySegment = new Map<
      string,
      typeof geometry.blockInstances
    >();
    geometry.blockInstances
      .filter((block) => block.course === 0)
      .forEach((block) => {
        const segmentId = block.segmentId ?? block.face;
        firstCourseBySegment.set(segmentId, [
          ...(firstCourseBySegment.get(segmentId) ?? []),
          block,
        ]);
      });
    ...
    firstCourseBySegment.forEach((blocks) => {
      expect(blocks.filter((block) => block.blockType === "half")).toHaveLength(
        0,
      );
  ...
  it("does not place half blocks at both ends of clean first-course rectangle spans", () => {
    ...
    expect(firstCourseDoubleHalfSegments(geometry.blockInstances)).toHaveLength(
      0,
    );
    expect(
      geometry.wallCmuLayout.warnings.some((warning) =>
        /redundant half blocks/i.test(warning),
      ),
    ).toBe(false);
  });
  ...
  it("prefers full stretchers on a 6m by 4m running-bond rectangle first course", () => {
    ...
    expect(
      geometry.wallCmuLayout.blocks.filter(
        (block) => block.course === 0 && block.blockType === "cut",
      ).length,
    ).toBeGreaterThan(0);
  ...
  it("records closed-loop placement provenance and station spans for generated units", () => {
    ...
    const placements = geometry.wallCmuLayout.unitPlacements.filter(
      (placement) => placement.courseIndex < 4,
    );
    ...
    expect(
      placements.every(
        (placement) =>
          placement.startStationMeters != null &&
          placement.endStationMeters != null,
      ),
    ).toBe(true);
    expect(placements.every((placement) => placement.kind != null)).toBe(true);
    expect(
      Array.from(new Set(placements.map((placement) => placement.source))),
    ).toEqual(expect.arrayContaining(["corner_assembly", "wall_run"]));
    expect(
      placements
        .filter((placement) => placement.source === "wall_run")
        .every(
          (placement) =>
            placement.kind === "stretcher" || placement.kind === "cut_block",
        ),
    ).toBe(true);
  });
  ...
  it("generates a 10m by 6m metric CMU rectangle with staggered structural cut units", () => {
    ...
    expect(
      geometry.blockInstances.some(
        (block) => block.blockType === "cut" || block.blockType === "end",
      ),
    ).toBe(true);
    expect(geometry.boundaryViolations).toHaveLength(0);
    expect(geometry.exteriorFootprint).toEqual(
      geometry.resolvedFootprint?.exteriorFacePolygon,
    );
  });
  ...
  it("uses full-dominant wall runs for the first four courses of modular rectangles", () => {
    ...
    cases.forEach(({ lengthMeters, widthMeters }) => {
      ...
      const firstFourRunUnits = geometry.wallCmuLayout.unitPlacements.filter(
        (placement) =>
          placement.courseIndex < 4 && placement.source === "wall_run",
      );
      const firstFourCorners =
        geometry.wallCmuLayout.cornerAssemblies?.filter(
          (assembly) => assembly.courseIndex < 4,
        ) ?? [];

      expect(firstFourRunUnits.length).toBeGreaterThan(0);
      expect(
        firstFourRunUnits.every(
          (placement) =>
            placement.kind === "stretcher" || placement.kind === "cut_block",
        ),
      ).toBe(true);
      ...
      for (let course = 0; course < 4; course += 1) {
        ...
        segmentIds.forEach((segmentId) => {
          const cornerHalfCount = geometry.wallCmuLayout.unitPlacements.filter(
            (placement) =>
              placement.courseIndex === course &&
              placement.segmentId === segmentId &&
              placement.kind === "half_block" &&
              placement.source === "corner_assembly",
          ).length;
      ...
      firstFourCorners
        ...
        .forEach((assembly) => {
          const nextCourse = firstFourCorners.find(
            (candidate) =>
              candidate.cornerId === assembly.cornerId &&
              candidate.courseIndex === assembly.courseIndex + 1,
          );
  ...
  it("resolves exterior footprint when an interior partition creates a T-junction", () => {
    ...
    const layoutWithPartition = addWallSegment(
      baseLayout,
      baseLayout.segments[3]!.startNodeId,
      0,
      0,
    );
    expect(layoutWithPartition.segments.length).toBe(5);

    const framedPreset = applyAutoFrameLayout({
      ...preset,
    ...
    expect(
      geometry.resolvedFootprint?.interiorFacePolygon.length,
    ...
    expect(framedPreset.frameSystem.columns.length).toBeGreaterThan(4);
    expect(
      geometry.resolvedRoofSystem?.gableEndSegmentIds.every((id) =>
        layoutWithPartition.segments.some((segment) => segment.id === id),
      ),
    ).toBe(true);
    const interiorSegmentId = layoutWithPartition.segments.at(-1)!.id;
    expect(geometry.resolvedRoofSystem?.gableEndSegmentIds).not.toContain(
      interiorSegmentId,
    );
    expect(
      geometry.blockInstances.some(
        (block) =>
          block.segmentId === interiorSegmentId &&
  ...
  it("stair-steps wall-run cuts near the middle without using terminal closures", () => {
    ...
    expect(
      geometry.wallCmuLayout.unitPlacements.every(
        (placement) => placement.source !== "terminal_closure",
      ),
    ).toBe(true);
      ...
      .reduce<string | undefined>((bestSegmentId, placement) => {
        const currentLength = Math.max(
          ...geometry.wallCmuLayout.unitPlacements
            .filter(
              (candidate) =>
                candidate.segmentId === placement.segmentId &&
                candidate.courseIndex === 0,
            )
        ...
        const bestLength = bestSegmentId
          ? Math.max(
              ...geometry.wallCmuLayout.unitPlacements
                .filter(
                  (candidate) =>
                    candidate.segmentId === bestSegmentId &&
                    candidate.courseIndex === 0,
                )
                .map((candidate) => candidate.endStationMeters ?? 0),
            )
    ...
    const segmentCuts = geometry.wallCmuLayout.unitPlacements
      .filter(
        (placement) =>
          placement.segmentId === segmentId && placement.kind === "cut_block",
      )
      .sort((a, b) => a.courseIndex - b.courseIndex);
    const firstFourCuts = [0, 1, 2, 3]
      .map((courseIndex) =>
        segmentCuts.find((placement) => placement.courseIndex === courseIndex),
      )
      .filter(
    ...
    const segmentWallLength = Math.max(
      ...geometry.wallCmuLayout.unitPlacements
        .filter(
          (placement) =>
            placement.segmentId === segmentId && placement.courseIndex === 0,
        )
    ...
    [0, 1, 2, 3].forEach((courseIndex) => {
      const courseCuts = segmentCuts
        ...
        .sort(
          (a, b) => (a.startStationMeters ?? 0) - (b.startStationMeters ?? 0),
        );
      for (let index = 1; index < courseCuts.length; index += 1) {
        expect(courseCuts[index].startStationMeters).toBeGreaterThan(
          (courseCuts[index - 1].endStationMeters ?? 0) + 0.05,
        );
      }
    });
    expect(
      new Set(firstFourCuts.map((cut) => cut.startStationMeters?.toFixed(3)))
        .size,
    ).toBeGreaterThan(1);
    expect(
      geometry.wallCmuLayout.warnings.some((warning) =>
        warning.includes("Stacked CMU head joint"),
      ),
    ).toBe(false);
  });
  ...
  it("chases non-modular exact dimensions with staggered wall-run cuts", () => {
    ...
    const xs =
      geometry.resolvedFootprint?.exteriorFacePolygon.map((point) => point.x) ??
      [];
    const zs =
      geometry.resolvedFootprint?.exteriorFacePolygon.map((point) => point.z) ??
      [];
    const courseZeroCuts = geometry.blockInstances.filter(
      (block) => block.course === 0 && block.blockType === "cut",
    );
  ...
  it("keeps exact dimensions with staggered cuts instead of terminal closures", () => {
    ...
    const firstCut = geometry.blockInstances.find(
      (block) => block.course === 0 && block.blockType === "cut",
    );
  ...
  it("resolves a 6m by 5m outside-face rectangle to exact exterior extents", () => {
    ...
    const resolved = resolveWallLayoutGeometry(layout, {
      wallThicknessMeters: 0.2,
    });
  ...
  it("normalizes a closed rectangle to one deterministic clockwise perimeter", () => {
    ...
    expect(
      perimeter?.segments.every((segment) => segment.lengthMeters > 0),
    ).toBe(true);
  });
  ...
  it("builds complete course plans before creating CMU placements", () => {
    ...
    expect(plans[0].cornerAssemblies[0].ownerSegmentId).not.toBe(
      plans[1].cornerAssemblies[0].ownerSegmentId,
    );
    plans.forEach((course) => {
      course.segmentPlans.forEach((plan) => {
        const length =
          perimeter?.segments.find(
            (segment) => segment.segmentId === plan.segmentId,
          )?.lengthMeters ?? 0;
  ...
  it("exposes one resolved footprint for walls and foundation geometry", () => {
    ...
    expect(geometry.resolvedFootprint?.exteriorFacePolygon).toEqual(
      geometry.exteriorFootprint,
    );
    expect(
      geometry.wallSegments.map((segment) => segment.lengthMeters),
    ).toEqual(expect.arrayContaining([6, 5]));
  });
  ...
  it("derives centerline and interior dimensions from outside-face basis and wall thickness", () => {
    ...
    expect(Math.max(...interiorXs) - Math.min(...interiorXs)).toBeCloseTo(
      5.6,
      6,
    );
    expect(Math.max(...interiorZs) - Math.min(...interiorZs)).toBeCloseTo(
      4.6,
      6,
    );
  });
  ...
  it("identifies four convex outside corners from ordered perimeter traversal", () => {
    ...
    expect(
      courseZeroAssemblies.every(
        (assembly) => assembly.cornerType === "convex_outside",
      ),
    ).toBe(true);
    expect(
      new Set(
        courseZeroAssemblies.map((assembly) => assembly.exteriorCornerPoint.x),
      ).size,
    ).toBe(2);
    expect(
      new Set(
        courseZeroAssemblies.map((assembly) => assembly.exteriorCornerPoint.z),
      ).size,
    ).toBe(2);
  });
  ...
  it("keeps outside-face CMU corners flush with the exterior dimensions", () => {
    ...
    const minX = Math.min(
      ...layout.blocks.map((block) =>
        block.face === "east" || block.face === "west"
          ? block.x - wallDepth / 2
          : block.x - block.lengthMeters / 2,
      ),
    );
    const maxX = Math.max(
      ...layout.blocks.map((block) =>
    ...
    const minZ = Math.min(
      ...layout.blocks.map((block) =>
        block.face === "north" || block.face === "south"
          ? block.z - wallDepth / 2
          : block.z - block.lengthMeters / 2,
      ),
    );
    const maxZ = Math.max(
      ...layout.blocks.map((block) =>
  ...
  it("does not double-generate overlapping corner blocks across joined wall faces", () => {
    ...
    const wallDepth = preset.wall.blockDepthMeters;
    const courseZeroBlocks = layout.blocks.filter(
      (block) => block.course === 0,
    );
  ...
  it("stack bond does not alternate courses", () => {
    ...
    expect(layout.warnings).toEqual(
      expect.arrayContaining([expect.stringMatching(/stack bond/i)]),
    );
  });
  ...
  it("reports placement-based module fit status from generated layouts", () => {
    ...
    const cutFit = generateCmuLayout({
      ...preset.wall,
      widthMeters: 5.13,
      openings: [],
    });
  ...
  it("rough opening allowance resolves through the CMU module grid", () => {
    ...
    const widerRoughLayout = generateCmuLayout({
      ...
      openings: preset.wall.openings.map((opening) =>
        opening.type === "door"
          ? { ...opening, roughOpeningAllowanceMeters: 0.1 }
          : opening,
      ),
    });

    expect(
      widerRoughLayout.roughOpenings[0].roughOpeningWidthMeters,
    ).toBeGreaterThanOrEqual(
      baseLayout.roughOpenings[0].roughOpeningWidthMeters,
    );
    const moduleLength = preset.wall.blockModule?.moduleLengthMeters ?? 0.4;
    const startRemainder =
      widerRoughLayout.roughOpenings[0].roughStartAlongMeters % moduleLength;
    expect(Math.min(startRemainder, moduleLength - startRemainder)).toBeCloseTo(
      0,
      6,
    );
    expect(widerRoughLayout.lintels[0].lengthMeters).toBeGreaterThanOrEqual(
      baseLayout.lintels[0].lengthMeters,
    );
  });
  ...
  it("derives trimmed jamb blocks where running bond units partially overlap a rough opening", () => {
    const preset = createFiveBySixCmuBuildingPreset();
    const layout = generateCmuLayout(preset.wall);
    const door = layout.roughOpenings.find(
    ...
    expect(
      new Set(trimmedJambs.map((block) => block.courseIndex)).size,
    ).toBeGreaterThan(1);
  ...
  it("running bond trims jamb blocks on alternating courses without leaving closure gaps", () => {
    ...
    const trimmedJambs = layout.blocks
      .filter(
        (block) =>
          ...
          typeof block.startAlongMeters === "number" &&
          typeof block.endAlongMeters === "number" &&
          (Math.abs(block.endAlongMeters! - door!.roughStartAlongMeters) <
            0.02 ||
            Math.abs(block.startAlongMeters! - door!.roughEndAlongMeters) <
              0.02),
      )
      .slice(0, 4);
    ...
    expect(
      new Set(
        trimmedJambs.map(
          (block) =>
            block.actualLengthMeters?.toFixed(3) ??
            block.lengthMeters.toFixed(3),
        ),
      ).size,
    ).toBeGreaterThan(0);
  });
  ...
  it("sums closure grout only for closures classified as grout fill", () => {
    ...
    const layout = generateCmuLayout({
      ...
      openings: preset.wall.openings.map((opening) =>
        opening.type === "door"
          ? { ...opening, roughOpeningAllowanceMeters: 0.02 }
          : opening,
      ),
    });
    const explicitClosureVolume = layout.openingCourseClosures.reduce(
      (sum, closure) =>
        sum +
        (closure.closureType === "grout_fill" ? (closure.groutVolume ?? 0) : 0),
      0,
    );

    expect(layout.openingGrout.closureGroutVolumeCubicMeters).toBeCloseTo(
      explicitClosureVolume,
      8,
    );
    expect(layout.openingGrout.closureGroutVolumeCubicMeters).toBeLessThan(
      layout.openingGrout.roughOpeningAreaSquareMeters *
        preset.wall.wallThicknessMeters,
    );
  });
});
...
function firstCourseDoubleHalfSegments(
  blocks: ReturnType<typeof generateDesignGeometry>["blockInstances"],
) {
  const bySegment = new Map<string, typeof blocks>();
  blocks
    .filter((block) => block.course === 0)
    .forEach((block) => {
      bySegment.set(block.segmentId, [
        ...(bySegment.get(block.segmentId) ?? []),
        block,
      ]);
    });
  return [...bySegment.entries()]
    .filter(([, segmentBlocks]) => {
      const ordered = [...segmentBlocks].sort(
        (a, b) => (a.stationMeters ?? 0) - (b.stationMeters ?? 0),
      );
      return (
        ordered[0]?.blockType === "half" &&
        ordered[ordered.length - 1]?.blockType === "half"
      );
    })
