/**
 * /dev/activity-preview — Assembly picker and rollup preview.
 *
...
import { instantiateFromAssemblySpec, type AssemblyInstantiationResult } from '../../domain/activityAssemblyInstantiation';
import type { ActivityLineItemTemplate, EstimateDivision, ProductionRate } from '../../domain/constructionActivityTypes';
import type { ActivityAssemblySpec } from '../../domain/activityAssemblyTypes';
...
export default function ActivityEstimatePreview() {
  ...
  const assembliesForDivision = useMemo(
    () => CA_ASSEMBLY_GROUPS.find((g) => g.divisionCode === selectedDivision)?.assemblies ?? [],
    [selectedDivision],
  );

  const assembly = useMemo(
    () => CA_ASSEMBLY_BY_ID.get(selectedAssemblyId),
    [selectedAssemblyId],
  );
  ...
  const handleDivisionChange = useCallback((div: string) => {
    ...
    const firstAssembly = CA_ASSEMBLY_GROUPS.find((g) => g.divisionCode === div)?.assemblies[0];
    if (firstAssembly) {
      ...
      <div className="max-w-6xl mx-auto mb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">
            Activity Estimate Preview
          </h1>
          <span className="text-sm text-gray-500">/ dev / activity-preview</span>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              ...
              const asm = CA_ASSEMBLY_BY_ID.get(item.asm);
