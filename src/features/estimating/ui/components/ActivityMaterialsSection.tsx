/**
 * Collapsible section listing material resources attached to a construction activity.
 * Renders inside the activity card "Estimate" tab.
 */
...
interface Props {
  activityId: string;
  projectId: string;
  resources: ActivityMaterialResource[];
  error: string | null;
  onAdd: () => void;
  onEdit: (resource: ActivityMaterialResource) => void;
  onRemove: UseActivityResourcesReturn['removeMaterial'];
}
...
export function ActivityMaterialsSection({ resources, error, onAdd, onEdit, onRemove }: Props) {
    ...
    <div className={`rounded-lg border ${BORDER_DEFAULT} overflow-hidden`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className={`text-sm font-medium ${TEXT_FOREGROUND}`}>Materials</span>
          {resources.length > 0 && (
        ...
        <div className="border-t border-slate-700/50">
          ...
          {resources.length === 0 && !error ? (
            <div className="px-4 py-3 text-center">
              <p className={`text-xs ${TEXT_SUBTLE}`}>No materials yet.</p>
              <button
                type="button"
                onClick={onAdd}
...
function ResourceRow({
  ...
  resource: ActivityMaterialResource;
  onEdit: () => void;
  onRemove: () => void;
}) {
