 ...
 *
 * Only called from the /dev/activity-preview page (DEV builds only) or from a
 * CLI script. Never called automatically on app startup.
 *
 * Uses UPSERT (ON CONFLICT DO UPDATE) so re-running is safe.
 */
import { supabase } from '../../../lib/supabase';
import type { ProductionRate, ConstructionActivityTemplate, ActivityLineItemTemplate } from '../domain/constructionActivityTypes';
import type { RepositoryResult } from '../infrastructure/estimateDbTypes';
import { DIV03_CONCRETE_SEED } from '../data/div03ConcreteSeeds';
import { DIV31_EARTHWORK_SEED } from '../data/div31EarthworkSeeds';
...
export async function seedAllDivisions(): Promise<SeedAllResult> {
  ...
  // Division 03
  const r03 = await seedProductionRates(DIV03_CONCRETE_SEED.productionRates);
  if (r03.error) {
    errors.push(`Div 03 rates: ${r03.error}`);
  } else {
    const t03 = await seedActivityTemplates(
      DIV03_CONCRETE_SEED.activities,
      DIV03_CONCRETE_SEED.lineItemGroups,
      '03',
    );
    if (t03.error) {
  ...
  // Division 31
  const r31 = await seedProductionRates(DIV31_EARTHWORK_SEED.productionRates);
  if (r31.error) {
    errors.push(`Div 31 rates: ${r31.error}`);
  } else {
    const allActivities = Object.values(DIV31_EARTHWORK_SEED).filter(
    ...
    const t31 = await seedActivityTemplates(
      allActivities,
      DIV31_EARTHWORK_SEED.lineItemGroups,
      '31',
    );
    if (t31.error) {
