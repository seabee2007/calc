...
import {
  ...
  SEABEE_ASSEMBLY_BY_ID,
  SEABEE_ASSEMBLY_GROUPS,
} from '../data/seabeeAssemblyRegistry';
import {
...
import { instantiateFromAssemblySpec } from '../domain/seabeeAssemblyInstantiation';

// ── Registry ─────────────────────────────────────────────────────────────────

describe('seabeeAssemblyRegistry', () => {
  it('has two division groups (31 Earthwork first, then 03 Concrete)', () => {
    expect(SEABEE_ASSEMBLY_GROUPS).toHaveLength(2);
    expect(SEABEE_ASSEMBLY_GROUPS[0].divisionCode).toBe('31');
    expect(SEABEE_ASSEMBLY_GROUPS[1].divisionCode).toBe('03');
  });

  it('contains five assemblies total', () => {
    const total = SEABEE_ASSEMBLY_GROUPS.reduce((n, g) => n + g.assemblies.length, 0);
    expect(total).toBe(5);
  });

  it('all assembly IDs are unique and findable by ID', () => {
    const allIds = SEABEE_ASSEMBLY_GROUPS.flatMap((g) => g.assemblies.map((a) => a.id));
    expect(new Set(allIds).size).toBe(allIds.length);
    for (const id of allIds) {
      expect(SEABEE_ASSEMBLY_BY_ID.has(id)).toBe(true);
    }
  });

  it('each assembly has at least one quantity input and one line item mapping', () => {
    for (const group of SEABEE_ASSEMBLY_GROUPS) {
...
describe('instantiateFromAssemblySpec — Place Slab on Grade', () => {
  const div03Rates = new Map(SEABEE_DIVISION_03_ALL_PRODUCTION_RATES.map((r) => [r.id, r]));
...
describe('instantiateFromAssemblySpec — Place Continuous Footing', () => {
  ...
  it('produces 3 line items from continuous footing assembly', () => {
    ...
    const result = instantiateFromAssemblySpec({
      ...
      division: SEABEE_DIVISION_03_CONCRETE,
      lineItemTemplates: SEABEE_CONTINUOUS_FOOTING_LINE_ITEMS,
      productionRates: div03Rates,
      projectId: 'test-project',
    });
...
describe('instantiateFromAssemblySpec — Clear and Grub Site', () => {
  const div31Rates = new Map(SEABEE_DIVISION_31_PRODUCTION_RATES.map((r) => [r.id, r]));

  it('produces 2 line items for 0.1 acre site', () => {
    const result = instantiateFromAssemblySpec({
      assembly: ASSEMBLY_CLEAR_AND_GRUB,
      userInputs: { siteAcres: 0.1 },
      division: SEABEE_DIVISION_31_EARTHWORK,
      lineItemTemplates: SEABEE_CLEAR_AND_GRUB_LINE_ITEMS,
...
describe('instantiateFromAssemblySpec — Excavate Footings', () => {
  ...
  it('produces 2 line items with correct quantities', () => {
    ...
    const result = instantiateFromAssemblySpec({
      ...
      division: SEABEE_DIVISION_31_EARTHWORK,
      lineItemTemplates: SEABEE_EXCAVATE_FOOTINGS_LINE_ITEMS,
      productionRates: div31Rates,
      projectId: 'test-project',
    });
...
describe('instantiateFromAssemblySpec — Backfill and Compact', () => {
  ...
  it('produces 1 line item', () => {
    const result = instantiateFromAssemblySpec({
      ...
      division: SEABEE_DIVISION_31_EARTHWORK,
      lineItemTemplates: SEABEE_BACKFILL_AND_COMPACT_LINE_ITEMS,
      productionRates: div31Rates,
      projectId: 'test-project',
    });
