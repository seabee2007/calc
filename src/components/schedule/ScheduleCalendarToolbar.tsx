    ...
    <div className={SCHEDULE_TOOLBAR}>
          ...
          <>
            <div className="flex items-center gap-0.5">
              <button
                type="button"
                onClick={onPrev}
                className="rounded-md p-1.5 text-[#4B5563] hover:bg-slate-100 dark:hover:bg-slate-800"
                aria-label="Previous"
              >
              ...
              <button
                type="button"
                onClick={onToday}
                className="rounded-md border border-[#E5E7EB] px-2 py-1 text-xs font-medium text-[#4B5563] hover:bg-slate-50 dark:border-slate-600 dark:text-slate-300"
              >
                Today
              </button>
              <button
      ...
      <div className="flex flex-wrap items-center gap-2">
        {showCalSwitcher && view === 'calendar' && (
          <div className="hidden flex-wrap gap-0.5 rounded-lg border border-[#E5E7EB] bg-[#F8FAFC] p-0.5 sm:flex dark:border-slate-600 dark:bg-slate-800">
              ...
              <button
                ...
                className={`${SCHEDULE_SUB_TAB} ${
                  cal === v
                    ...
                    : 'text-[#4B5563] hover:bg-white dark:text-slate-300 dark:hover:bg-slate-700'
                }`}
              >
                {CAL_LABELS[v]}
              </button>
        ...
        <div className="relative" ref={menuRef}>
          <button
            ...
            className="rounded-md border border-[#E5E7EB] p-2 text-[#4B5563] hover:bg-slate-50 dark:border-slate-600 dark:hover:bg-slate-800"
            aria-label="More views"
          >
            <MoreHorizontal className="h-4 w-4" />
          </button>
          {menuOpen && (
            <div className="absolute right-0 top-full z-50 mt-1 min-w-[160px] rounded-lg border border-[#E5E7EB] bg-white py-1 shadow-lg dark:border-slate-600 dark:bg-slate-800">
              {view !== 'calendar' && (
                <button
                  type="button"
              ...
              {view === 'calendar' && (
                <div className="border-t border-[#E5E7EB] px-2 py-2 dark:border-slate-600 sm:hidden">
                  <p className="mb-1 text-[10px] font-semibold uppercase text-[#6B7280]">View</p>
                  <div className="flex flex-wrap gap-1">
                    {CALENDAR_SUB_VIEWS.map((v) => (
                      <button
                        ...
                        className={`${SCHEDULE_SUB_TAB} ${
                          cal === v ? 'bg-[#2563EB] text-white' : 'text-[#4B5563]'
                        }`}
                      >
                        {CAL_LABELS[v]}
                      </button>
